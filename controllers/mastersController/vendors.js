module.exports = (db) => {
    return {
      ...
      searchVendors: (req, res) => {
        const { name_primary, name_secondary, name_kana, id, classification1, classification2 } = req.query; // Assuming the query is passed as a query parameter, e.g., ?name=example
        let sql;
        let params = [];
      
        if ((name_primary || name_secondary || name_kana || id || classification1 || classification2).trim() !== "") {
          sql = `
              SELECT * FROM vendors 
              WHERE name_primary LIKE ? 
